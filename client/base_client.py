...
class BaseClient(ABC):
    ...
    TERMINAL_RESET_SEQUENCE = b'\x1b[?2004l\x1b[?25h\x1b[?1049l'
    ...
    def _write_terminal_reset_sequences(self):
        """在退出时尽量恢复终端到普通 shell 状态"""
        if not sys.stdin.isatty():
            return
        try:
            sys.stdout.buffer.write(self.TERMINAL_RESET_SEQUENCE)
    ...
    async def _cleanup(self):
        ...
        self._write_terminal_reset_sequences()
...
class BaseWSClient(BaseClient):
    ...
    async def send_control(self, action: str, timeout: float = 30.0) -> dict:
        ...
        """
        from client.remote_client import connect
