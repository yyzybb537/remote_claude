...
class TestBaseClientCleanup:
    ...
    @pytest.mark.anyio
    async def test_cleanup_writes_terminal_reset_sequences_after_restore(self, monkeypatch):
        """测试 cleanup 会在恢复终端后输出复位序列"""
        client = MockClient()
        client.running = True
        client._connected = True
        ...
        def fake_restore_terminal():
            events.append("restore")

        def fake_write_reset_sequences():
            events.append("write_reset")

        monkeypatch.setattr(client, "_restore_terminal", fake_restore_terminal)
        monkeypatch.setattr(client, "_write_terminal_reset_sequences", fake_write_reset_sequences, raising=False)

        await client._cleanup()

        assert "restore" in events
        assert "write_reset" in events
        assert events.index("restore") < events.index("write_reset")
