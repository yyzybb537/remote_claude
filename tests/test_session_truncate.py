...
from utils.runtime_config import State, SessionState, QuickCommand
...
def test_safe_filename_with_slash():
    ...
    print(f"[OK] 路径分隔符替换: {name} -> {result}")
...
def test_safe_filename_truncate():
    ...
    print(f"[OK] 超长截断（{_MAX_FILENAME}字符限制）: 长度 {len(name)} -> {len(result)}")
...
def test_safe_filename_md5_fallback():
    ...
    print(f"[OK] MD5 回退: 长度 200 单部分 -> {result[:20]}...")
...
def test_safe_filename_consecutive_underscores():
    ...
    expected = "a_b_c"
    assert result == expected, f"预期 '{expected}'，实际 '{result}'"
    print(f"[OK] 连续下划线合并: {name} -> {result}")
    ...
    assert result == expected, f"预期 '{expected}'，实际 '{result}'"
    print(f"[OK] 路径连续分隔符合并: {name} -> {result}")
    ...
    expected = "path_to_project"
    assert result == expected, f"预期 '{expected}'，实际 '{result}'"
    print(f"[OK] 混合分隔符合并: {name} -> {result}")
...
def test_safe_filename_strip_underscores():
    ...
    assert result == expected, f"预期 '{expected}'，实际 '{result}'"
    print(f"[OK] 首尾下划线去除: {name} -> {result}")
    ...
    expected = "test_session"
    assert result == expected, f"预期 '{expected}'，实际 '{result}'"
    print(f"[OK] 路径开头斜杠处理: {name} -> {result}")


def test_state_session_path():
    """测试会话路径存储"""
    state = State()
    ...
    # 获取路径
    result = state.get_session_path("myapp_src")
    assert result == "/Users/dev/projects/myapp/src", f"预期 '/Users/dev/projects/myapp/src'，实际 '{result}'"

    # 不存在的路径
    result = state.get_session_path("nonexistent")
    assert result is None, f"预期 None，实际 '{result}'"

    print("[OK] 会话路径存储和获取")


def test_state_session_limit():
    """测试会话数量限制警告（不阻塞）"""
    state = State()

    # 设置大量会话（超过限制会输出警告但不阻塞）
    for i in range(10):
        state.set_session_path(f"session_{i}", f"/path/to/session_{i}")

    # 验证会话已保存
    assert len(state.sessions) == 10, f"预期 10 个会话，实际 {len(state.sessions)}"
    print(f"[OK] 会话数量限制处理: {len(state.sessions)} 个会话")
...
def test_state_json_roundtrip():
    ...
    # 创建状态
    state = State()
    state.set_session_path("test_session", "/path/to/test")
    state.set_lark_chat_id("test_session", "oc_123")
    ...
    state2 = State.from_dict(data2)

    # 验证
    assert state2.get_session_path("test_session") == "/path/to/test"
    assert state2.get_lark_chat_id("test_session") == "oc_123"
...
def test_max_filename_platform():
    ...
    print(f"[OK] 当前平台: {system}, _MAX_FILENAME: {_MAX_FILENAME}")
...
def run_all_tests():
    ...
    tests = [
        ...
        test_state_session_path,
        test_state_session_limit,
