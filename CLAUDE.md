...
**文件锁**：写入 `runtime.json` 时使用文件锁 `runtime.json.lock` 保护并发写入。锁文件包含用途说明、进程 PID 和创建时间。

#### 配置迁移

- 旧配置文件 `lark_group_mapping.json` 会自动迁移到 `runtime.json`
- 旧版 `runtime.json` 中的 `ui_settings` 会自动迁移到 `config.json`
- 残留的 `.bak` 备份文件会在飞书客户端启动时提示用户处理
