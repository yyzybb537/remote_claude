...
class WebSocketHandler:
    ...
    def _do_list(self) -> ControlResponseMessage:
        """列出当前远程入口可见的会话集合。

        当前 WebSocket 入口绑定单个 session，因此这里返回当前 session 的状态列表。
        """
        try:
            sessions = [{
            ...
            return ControlResponseMessage(True, json.dumps({"sessions": sessions}))
        except Exception as e:
            logger.error(f"列出会话失败: {e}")
            return ControlResponseMessage(False, f"列出会话失败: {e}")
