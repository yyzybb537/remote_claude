...
| 命令 | 说明 |
|------|------|
| `/menu` | 显示会话菜单，可启动/连接/管理会话 |
| `/list` | 列出可用会话（等价于 `/menu`） |
| `/start <会话名> [路径]` | 启动新会话（可选指定工作目录） |
| `/attach <会话名>` | 连接到已有会话 |
| `/detach` | 断开当前会话（保持卡片不变） |
| `/kill <会话名>` | 终止会话 |
| `/status` | 显示当前连接状态 |
| `/ls [路径]` | 列出远程目录内容 |
| `/tree [路径]` | 以树形结构列出远程目录 |
| `/new-group <会话名>` | 创建专属群聊，多人共用同一 Claude 会话 |
| `/help` | 显示帮助信息 |
...
如果已有活跃会话绑定，直接发送文本消息即可，无需 `/` 前缀。消息会自动转发给 Claude。

**会话绑定持久化**：私聊/群聊与会话的绑定关系会自动保存，重启飞书客户端后自动恢复。
