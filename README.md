...
3. 用`cla`或`cl`启动一次claude（会附带启动飞书客户端，如果之前已经启动过，需要重启飞书客户端: "uv run python3 remote_claude.py lark restart" ） 
...
```bash
uv run python3 remote_claude.py start <会话名>     # 启动新会话
uv run python3 remote_claude.py attach <会话名>    # 连接现有会话
uv run python3 remote_claude.py list               # 查看所有会话
uv run python3 remote_claude.py kill <会话名>      # 终止会话
```

### 飞书客户端

```bash
uv run python3 remote_claude.py lark start         # 启动（后台运行）
uv run python3 remote_claude.py lark stop          # 停止
uv run python3 remote_claude.py lark restart       # 重启
uv run python3 remote_claude.py lark status        # 查看状态
```
